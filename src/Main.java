import java.io.IOException;
import java.util.Scanner;
import java.io.FileWriter;
import java.io.FileReader;
import java.io.IOException;
import java.io.File;
import java.io.FileNotFoundException;

public class Main {

	public static void main(String[] args) throws IOException {
		System.out.println("Welcome to Mentcare!");
		Scanner scan = new Scanner(System.in);
//		s.writeInfo("Logan Farrington", 21, "29619 n 138th st", "602-810-4759", "ldfarrin@asu.edu");
		System.out.println("Please enter your 'name,age,address,phone,email.'");
		String input = scan.next();
		String[] inputArr = input.split(",");
		System.out.println(inputArr[0]);
		signup(inputArr[0], Integer.parseInt(inputArr[1]), inputArr[2], inputArr[3], inputArr[4]);
		
	}
	
		
	public static void signup(String name, int age,String address, String phone, String email) {
		File infoFile = new File("infoFile.txt");
		try {
			FileWriter fw = new FileWriter(infoFile, true); //true means it will append at the bottom.
			fw.write(name + "," + age + "," + address + "," + phone + "," + email + "\n");
			
			fw.close();
			
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}

}
